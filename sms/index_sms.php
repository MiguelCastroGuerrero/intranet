...
<div class="page-header">
  <h2>SMS <small> Env�o de mensajes</small></h2>
<?php
if(strlen($unidad)>1){
...
if($submit0 == "Enviar SMS")
{
	if(empty($causa)){
		echo '<div align="center"><div class="alert alert-danger alert-block fade in">
			...
			<h4>ATENCI�N:</h4>
No has seleccionado ninguna Causa del Mensaje.<br />Vuelve atr�s, selecci�nala e int�ntalo de nuevo.
          </div></div>';
		  exit();
}
if(empty($text)){
		echo '<div align="center"><div class="alert alert-danger alert-block fade in">
            <button type="button" class="close" data-dismiss="alert">&times;</button>
			<h4>ATENCI�N:</h4>
No has escrito ning�n texto para el Mensaje.<br />Vuelve atr�s, redacta el texto e int�ntalo de nuevo.
          </div></div>';
		  exit();
}
  	if(strlen($text) > "2")
...
No has seleccionado ning�n alumno para el env�o de SMS.<br />Vuelve atr�s, selecci�nalo e int�ntalo de nuevo.
          </div></div>';
		  exit();
}
...
El mensaje SMS se ha enviado correctamente a los siguientes alumnos: '.$alumno_nombre.'.<br>Una nueva acci�n tutorial ha sido tambi�n registrada.
			...
			<div class=\"alert alert-error\">
				<strong>Error:</strong> No se pudo enviar el SMS al tel�fono (+34) ".$num_movil.". Corrija la informaci�n de contacto del alumno/a en S�neca e importe los datos nuevamente.
			</div>
			<br>";
		}
	}
...
$query0="insert into mens_texto (asunto, texto, origen, destino) values ('Env�o de SMS desde Jefatura de Estudios a los padres de ".$alumnos_nombre." con el siguiente texto:<< ".$observaciones.">>','".$observaciones."','".$profe."', '$alumnos_sms')";
//echo "$query0<br>";
mysqli_query($db_con, $query0);
$id0 = mysqli_query($db_con, "select id from mens_texto where asunto like 'Env�o de SMS desde Jefatura de Estudios a los padres de ".$alumnos_nombre."%' and texto = '$observaciones' and origen = '$profe'");
...
<form method="post" action="index.php" name="nameform" class="form-vertical">
 ...
 <?php if(stristr($_SESSION['cargo'],'8') == TRUE){?>
		    ...
		    <option>Orientaci�n acad�mica y profesional</option>
...
<div class="well">
<div class='form-group'>
<label>Selecci�n de Alumnos<?php echo "<span class='text-info'>: $unidad</span>"; ?></label>
        <?php
