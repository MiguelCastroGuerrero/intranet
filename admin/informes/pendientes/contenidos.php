...
if (isset($_GET['borrar']) AND $_GET['borrar'] == 1) {
	$result = mysqli_query($db_con,"delete from informe_pendientes_contenidos where id_informe= '".$_GET['id_informe']."' and id_contenido= '".$_GET['id_contenido']."'");
	if (! $result) {
...
$result = mysqli_query($db_con, "SELECT `id_contenido`, `unidad`, `titulo`, `contenidos`, `actividades` FROM `informe_pendientes_contenidos` WHERE `id_informe` = '".$id_informe."' ORDER BY `id_contenido` ASC");
...
if (isset($_POST['crear_contenido'])) {
	...
	$result = mysqli_query($db_con, "INSERT INTO `informe_pendientes_contenidos` (`id_informe`, `unidad`, `titulo`, `contenidos`, `actividades`) VALUES ('".$id_informe."', '".$unidad."', '".$titulo."', '".$contenidos."', '".$actividades."')");
	if (! $result) {
...
if (isset($_POST['actualizar_contenido'])) {
	...
	$result = mysqli_query($db_con, "update `informe_pendientes_contenidos` set unidad= '".$unidad."', titulo= '".$titulo."', contenidos= '".$contenidos."', actividades= '".$actividades."' where id_informe='".$id_informe."' and id_contenido='".$id_contenido."'");
	if (! $result) {
		$msg_error = "No se ha podido añadir contenido a este informe. Error: ".mysqli_error($db_con);
	}
	else {
...
if (isset($_GET['editar']) AND isset($_GET['id_contenido'])) {
	$result = mysqli_query($db_con, "select * from informe_pendientes_contenidos where id_informe = '".$_GET['id_informe']."' and id_contenido = '".$_GET['id_contenido']."'");
	if ($result) {
...
?>
	...
	<div class="container page-header">
		<h2>Informe individual de evaluación para materias pendientes  <small><br><?php echo $datos_informe['asignatura']; ?><br><?php echo $datos_informe['unidad']; ?></small></h2>
	</div>
				...
				<table class="table table-striped">
					...
					<tbody>
						...
						<tr>
							...
							<td>
								<a href="//<?php echo $config['dominio']; ?>/intranet/admin/informes/pendientes/contenidos.php?editar=1&id_informe=<?php echo $id_informe; ?>&id_contenido=<?php echo $contenido_informe['id_contenido']; ?>"><span class="far fa-edit fa-fw fa-lg"></span></a>
								<br><br>
								<a href="//<?php echo $config['dominio']; ?>/intranet/admin/informes/pendientes/contenidos.php?borrar=1&id_informe=<?php echo $id_informe; ?>&id_contenido=<?php echo $contenido_informe['id_contenido']; ?>" data-bb='confirm-delete'><span class="far fa-trash-alt fa-fw fa-lg"></span></a>
							</td>
						</tr>
						<?php endforeach; ?>
						<?php } else{ ?>
							<?php while ($contenido_informe = mysqli_fetch_array($result_cont)): 
								mysqli_query($db_con,"insert into informe_pendientes_contenidos (id_informe,unidad, titulo, contenidos,actividades,id_contenido) VALUES ('".$id_informe."','".$contenido_informe['unidad']."','".$contenido_informe['titulo']."','".$contenido_informe['contenidos']."','".$contenido_informe['actividades']."','".$contenido_informe['id_contenido']."')");
							?>
							<tr>
								<td><strong><?php echo $contenido_informe['unidad']; ?></strong><br><?php echo $contenido_informe['titulo']; ?></td>
								<td><?php echo $contenido_informe['contenidos']; ?></td>
