...
/*	if ($fechainicio == '00-00-0000' or $fechafin == '00-00-0000') {
        ...
        if(empty($inicio_aula) OR empty($fin_aula) OR empty($convivencia))
        {
			...
			<h5>ATENCIÓN:</h5>No has escrito datos en <u>todos</u> los campos del formulario del Aula de Convivencia. Inténtalo de nuevo.</div></div>';
        } 
        elseif (strstr($inicio_aula,"-")==FALSE OR strstr($fin_aula,"-")==FALSE) 
        {
	        echo '<div align="center"><div class="alert alert-danger alert-block fade in">
            <button type="button" class="close" data-dismiss="alert">&times;</button>
			<h5>ATENCIÓN:</h5>El formato de las fechas no es correcto. Lo correcto es "dia-mes-año" (p.ej. 15-10-2009). Inténtalo de nuevo.</div></div>';
        }
        else 
        {
            ...
            if ($config['mod_sms'] and $mens_movil == 'envia_sms') 
            {
                if((substr($tfno,0,1)=="6" or substr($tfno,0,1)=="7" or substr($tfno_u,0,1)=="6" or substr($tfno_u,0,1)=="7")) 
                {
                    ...
                    $message2 = "Por favor, pongase en contacto con nosotros.";
            ...
            if(!(empty($tareas))) 
            {
                ...
                {
                    mysqli_query($db_con, "INSERT tareas_alumnos (CLAVEAL,APELLIDOS,NOMBRE,unidad,FECHA,DURACION,PROFESOR, FIN) VALUES ('$claveal','$apellidos','$nombre','$unidad', '$inicio_aula','$convivencia','$tutor', '$fin_aula')") 
                    or die ("Error: no se ha podido activar el informe:".mysqli_error($db_con));
                }
                else 
...
if($submit) 
{
	...
	else 
    {
	    if(empty($inicio) OR empty($fin) OR empty($expulsion))
        {
		    echo '<div align="center"><div class="alert alert-danger alert-block fade in">
            <button type="button" class="close" data-dismiss="alert">&times;</button>
			<h5>ATENCIÓN:</h5>No has escrito datos en <u>todos</u> los campos del formulario de expulsión. Inténtalo de nuevo.
            </div></div>';
	    }
		elseif (strstr($inicio,"-")==FALSE OR strstr($fin,"-")==FALSE) 
        {
		    echo '<div align="center"><div class="alert alert-danger alert-block fade in">
                <button type="button" class="close" data-dismiss="alert">&times;</button>
			    <h5>ATENCIÓN:</h5>>El formato de las fechas no es correcto. Lo correcto es "dia-mes-año" (p.ej. 15-10-2009). Inténtalo de nuevo.
                </div></div>';
	    }
	    else 
        {
            if($inicio) 
                $inicio1 = explode("-",$inicio); $inicio = $inicio1[2] . "-" . $inicio1[1] ."-" . $inicio1[0];
            
            if($fin)  
                $fin1 = explode("-",$fin); $fin = $fin1[2] . "-" . $fin1[1] ."-" . $fin1[0];
            ...
            if ($config['mod_sms'] and $mens_movil == 'envia_sms') 
            {
                ...
                {	
                    if ($mens_movil=="envia_sms") 
                    {
                        ...
                        {
                            $message = "Le comunicamos que su hijo/a va a ser expulsado del Centro. Por favor, pongase en contacto con nosotros.";
	                        ...
	                        if(strlen($mobile) == 9) 
                            {
                                ...
                                if ($smsSent->result == "OK") {
                                    ...
                                    $accion = "Envío de SMS";
                                    mysqli_query($db_con, "insert into tutoria (apellidos, nombre, tutor,unidad,observaciones,causa,accion,fecha,jefatura,claveal) values ('".$apellidos."','".$nombre."','".$tutor."','".$unidad."','".$message."','".$causa."','".$accion."','".$fecha2."','1','".$claveal."')");
                                }

	                        }
	                        else 
		                        echo "<div class=\"alert alert-error\"><strong>Error:</strong> No se pudo enviar el SMS al teléfono (+34) ".$mobile.". Corrija la información de contacto del alumno/a en Séneca e importe los datos nuevamente.</div><br>";
                        }
                    }
                }
            }
