...
if(isset($_POST['suma_columnas']) and $_POST['suma_columnas']=="Guardar resultado como columna"){
	...
	$curso=$_POST['curso'];
	$texto="Suma de columnas: $id_cols";
	$nombre="Suma de columnas: $id_cols";
	$tipo="Suma";
...
VALUES ( '$pr',  '$fecha',  '$nombre',  '$texto',  '$asignatura', '$curso', '$orden', '$tipo', '#fff')";
	//echo $sql."<br>";
...
foreach ($_POST as $id => $valor) {
	if (is_numeric($id) and strlen($id)<5){
		...
		$t='La ponderaci&oacute;n usada es 1';
...
echo "<th nowrap style='background-color:#fff'>
<div style='width:40px;height:100px;'>
<div class='Rotate-90'><span class='text-warning text-lowercase'>Suma de notas</span> </div>
</div> </th></thead>";
...
foreach ($_POST as $id => $valor2) {
	...
	<legend>Columnas en la suma</legend> 
	<?php
$nid='';
$name="";
foreach ($_POST as $id => $valor) {
...
if (mysqli_num_rows($colum0) > 0) {
	...
	echo "<tr><th colspan='2'>Columnas</th>";
}
while ($colum1 = mysqli_fetch_array($colum0)) {
	...
	?> <?php
	echo"<input type=hidden name='$colum1[0]' value='$pon0[0]'/>";
	echo "</tr>";
...
<br />
<input name="suma_columnas" type="submit"	value="Guardar resultado como columna" class="btn btn-info" />
	<br>
<br>
